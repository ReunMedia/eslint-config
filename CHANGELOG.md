# Changelog

All notable changes to this project will be documented in this file.

The format is based on [Keep a Changelog](https://keepachangelog.com/en/1.1.0/)
and [Common Changelog](https://common-changelog.org/), and this project adheres
to [Semantic Versioning](https://semver.org/spec/v2.0.0.html).

## [1.8.0] - 2025-11-03

### Added

- Export `useGitignore()` separately for manual configuration usage

## [1.7.0] - 2025-11-03

### Added

- Export individual configs separately so that they can be manually imported
- Allow prefixing unused variables with `_` to ignore errors

### Changed

- Update documentation to use `defineConfig()` instead of deprecated
  `tseslint.config()`

## [1.6.0] - 2025-07-10

### Added

- Astro configuration

## [1.5.0] - 2025-07-09

Initial release based on [ESLint config version
`1.5.0`](https://github.com/ReunMedia/project-templates/blob/637552a79e6add952d6bf9c4a3520d741954cde9/base/webdev/frontend/eslint.config.mjs)
in Reun Media Project Templates.

[1.8.0]: https://github.com/ReunMedia/eslint-config/releases/tag/1.8.0
[1.7.0]: https://github.com/ReunMedia/eslint-config/releases/tag/1.7.0
[1.6.0]: https://github.com/ReunMedia/eslint-config/releases/tag/1.6.0
[1.5.0]: https://github.com/ReunMedia/eslint-config/releases/tag/1.5.0
